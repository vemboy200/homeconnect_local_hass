...
Once an appliance is set up, you can export its profile two ways:

- **Safe** (the two XML files, renamed to `{brand}_{model}`, no encryption key or other sensitive data): Settings → Devices & Services → Home Connect Local → the appliance's device page → **Download Diagnostics**. This is a real, native Home Assistant download button (the same one every integration's diagnostics data uses) - one click, no link to open, no notification to find. This is what you want for [Requesting a New Feature](support-and-troubleshooting.md#requesting-a-new-feature).
- **Full** (the same two XML files plus the local encryption key, in the same shape as the Profile Downloader ZIP, re-importable via Upload Profile File): the device page's gear/settings icon instead. Meant for transferring an appliance to another system - not just Home Assistant, but other local-control projects like [openHAB's Home Connect Direct Binding](https://community.openhab.org/t/home-connect-direct-binding-no-cloud/160857) or the [Homey Home Connect (Local) app](https://homey.app/en-us/app/codes.lucasvdh.homeconnect/Home-Connect-(Local)/test/) - not something most users need. Contains your real encryption key, so it's deliberately not offered as a download link at all - it's written to a `homeconnect_ws_export` folder in your config directory instead, retrievable via Samba, SSH, or the File Editor add-on, the same access boundary as any other file in your HA config.
