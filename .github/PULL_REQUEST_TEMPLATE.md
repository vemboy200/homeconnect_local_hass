...
- [ ] If this adds/changes an entity: the `translations/en.json` and `translations/de.json` files have been updated (English is the reference locale, and German is required too: Germany has the most users of this integration. CI fails if `de.json` is missing a key from `en.json`. Other languages are welcome but not required, a maintainer can fill those in)
