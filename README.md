...
> [!NOTE]
> This README covers the basics. For extended documentation (in-depth troubleshooting, automation examples, supported functions) see the [wiki](https://github.com/vemboy200/homeconnect_local_hass/wiki).
...
- Unlike appliance types such as dishwashers, which stay connected over Wi-Fi even while powered off, and only lose the ability to write to specific power-gated entities, standalone washing machines and dryers disconnect from Wi-Fi entirely when powered off. This is expected, normal behavior for these appliance types, not a bug. Confirmed on a Bosch washer (WAU28PEHSN) and a Siemens dryer, see [issue #7](https://github.com/vemboy200/homeconnect_local_hass/issues/7) for the full discussion. As of 1.7.0, the integration tells this apart from a genuine fault: if the appliance sent a clean disconnect (WebSocket close code 1000, meaning it deliberately cut its own Wi-Fi rather than dropping unexpectedly) right before going quiet, its entities stay available and keep reporting their last-known state through the outage instead of going `unavailable`. An abnormal drop (any other close code, or none seen yet) still correctly marks them unavailable, same as any other appliance. Whether washer/dryer combo units behave the same way is unconfirmed. At least one combo model (see the program-selection limitation below) stays connected while powered off instead, closer to the dishwasher pattern, so combos don't get this treatment.
- On at least some Washer/Dryer/WasherDryer models, you cannot remotely **select** which program runs. Only the official Home Connect app can do that, even though the appliance's own profile file claims that resource is read-write. Starting, pausing, and otherwise controlling whatever program is already selected works fine from Home Assistant. Confirmed on a Bosch WNC254A0BY (WasherDryer), see [issue #9](https://github.com/vemboy200/homeconnect_local_hass/issues/9) for the full investigation. Unlike other unavailable states (e.g. an appliance being powered off), there's no state or schema field that predicts this ahead of time, the appliance stays connected, reports itself as writable, and the entity looks and behaves like a normal, working selector right up until you try to change it and get a rejection back. Practical workaround: pre-select the program you want via the app or the appliance's own dial, then use Home Assistant to trigger the actual start (e.g. at a cheap-energy price window) instead of trying to automate program choice too.
